...
int main(int argc, char *argv[])
{
    ...
    char c;
     f=fopen("test.txt","r");
    while((c=getc(f))!=EOF)			//Counting No of words in the filE, IGNORE
	{	
		fscanf(f , "%s" , buffer);
		if(isspace(c)||c=='\t')
		words++;	
    ...
    printf("\nSuccess : The file 1 was sent successfully\n");
  /*  
    FILE *f2;
    f2=fopen("glad2.txt","r");
    char ch2;
    while(ch2 != EOF)
   {
     ...
     fscanf(f2 , "%s" , buffer2);
     //printf("%s\n" , buffer);	//Ignore
     write(sockfd,buffer2,512);
     ch2 = fgetc(f2);
   }
 printf("The file 2 was sent successfully");
  */  
    close(sockfd);
    return 0;
}
